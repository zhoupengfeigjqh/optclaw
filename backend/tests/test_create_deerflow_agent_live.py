"""Live integration tests for create_optclaw_agent.
...
def test_minimal_agent_responds():
    """create_optclaw_agent(model) produces a graph that returns a response."""
    from optclaw.agents.factory import create_optclaw_agent

    model = _make_model()
    graph = create_optclaw_agent(model, features=None, middleware=[])
...
def test_agent_with_custom_tool():
    ...
    model = _make_model()
    graph = create_optclaw_agent(model, tools=[add], middleware=[])
...
def test_features_mode_middleware_chain():
    ...
    graph = create_optclaw_agent(model, features=feat)
