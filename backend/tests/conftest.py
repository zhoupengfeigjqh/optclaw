...
# Make 'app' and 'optclaw' importable from any working directory
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "scripts"))

# Break the circular import chain that exists in production code:
#   optclaw.subagents.__init__
#     -> .executor (SubagentExecutor, SubagentResult)
#       -> optclaw.agents.thread_state
#         -> optclaw.agents.__init__
#           -> lead_agent.agent
#             -> subagent_limit_middleware
#               -> optclaw.subagents.executor  <-- circular!
#
# By injecting a mock for optclaw.subagents.executor *before* any test module
...
sys.modules["optclaw.subagents.executor"] = _executor_mock
