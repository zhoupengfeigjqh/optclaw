...
import optclaw.config.app_config as app_config_module
from optclaw.agents.checkpointer import get_checkpointer, reset_checkpointer
from optclaw.config.checkpointer_config import (
    CheckpointerConfig,
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("optclaw.agents.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
...
class TestAsyncCheckpointer:
    ...
    async def test_sqlite_creates_parent_dir_via_to_thread(self):
        ...
        from optclaw.agents.checkpointer.async_provider import make_checkpointer
        ...
        with (
            patch("optclaw.agents.checkpointer.async_provider.get_app_config", return_value=mock_config),
            patch.dict(sys.modules, {"langgraph.checkpoint.sqlite.aio": mock_module}),
            patch("optclaw.agents.checkpointer.async_provider.asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread,
            patch(
                "optclaw.agents.checkpointer.async_provider.resolve_sqlite_conn_str",
...
# ---------------------------------------------------------------------------
# OptClawClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """OptClawClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("optclaw.client.get_app_config", return_value=config_mock),
            patch("optclaw.client.create_agent", side_effect=fake_create_agent),
            patch("optclaw.client.create_chat_model", return_value=MagicMock()),
            patch("optclaw.client._build_middlewares", return_value=[]),
            patch("optclaw.client.apply_prompt_template", return_value=""),
            patch("optclaw.client.OptClawClient._get_tools", return_value=[]),
        ):
            client = OptClawClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = OptClawClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
