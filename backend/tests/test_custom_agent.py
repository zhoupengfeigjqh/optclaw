...
from optclaw.config.agents_api_config import AgentsApiConfig, get_agents_api_config, set_agents_api_config
...
class TestAgentConfig:
    def test_minimal_config(self):
        from optclaw.config.agents_config import AgentConfig
...
class TestLoadAgentConfig:
    def test_load_valid_config(self, tmp_path):
        ...
        with patch("optclaw.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
...
class TestLoadAgentSoul:
    def test_reads_soul_file(self, tmp_path):
        ...
        with patch("optclaw.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from optclaw.config.agents_config import AgentConfig, load_agent_soul
...
class TestListCustomAgents:
    def test_empty_when_no_agents_dir(self, tmp_path):
        with patch("optclaw.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from optclaw.config.agents_config import list_custom_agents
...
class TestMemoryFilePath:
    def test_global_memory_path(self, tmp_path):
        ...
        from optclaw.agents.memory.storage import FileMemoryStorage
        from optclaw.config.memory_config import MemoryConfig

        with (
            patch("optclaw.agents.memory.storage.get_paths", return_value=_make_paths(tmp_path)),
            patch("optclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_path="")),
        ):
...
def agent_client(tmp_path):
    ...
    with patch("optclaw.config.agents_config.get_paths", return_value=paths_instance), patch.object(agents_router, "get_paths", return_value=paths_instance):
