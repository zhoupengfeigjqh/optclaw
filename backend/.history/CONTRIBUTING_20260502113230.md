# Contributing to optclaw Backend

Thank you for your interest in contributing to optclaw! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/opt-claw.git
   cd opt-claw
   ```
...
```python
import pytest
from optclaw.models.factory import create_chat_model
...
1. Create tool in `packages/harness/optclaw/tools/builtins/` or `packages/harness/optclaw/community/`:

```python
# packages/harness/optclaw/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: optclaw.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/optclaw/agents/middlewares/`:

```python
# packages/harness/optclaw/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/optclaw/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/optclaw/config/app_config.py` with new fields
...
Thank you for contributing to optclaw!
