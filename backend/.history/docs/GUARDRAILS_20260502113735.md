...
The simplest option. Ships with optclaw. Block or allow tools by name. No external packages, no passport, no network.
...
guardrails:
  enabled: true
  provider:
    use: optclaw.guardrails.builtin:AllowlistProvider
    config:
...
aport setup --framework optclaw
```

This creates:
- `~/.aport/optclaw/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/optclaw/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; optclaw doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the optclaw middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start optclaw and ask: "Create a file called test.txt with content hello"
...
class MyGuardrailProvider:
    ...
    def evaluate(self, request):
        from optclaw.guardrails.provider import GuardrailDecision, GuardrailReason
...
2. Add the config
3. Start optclaw and ask: "Use bash to delete test.txt"
...
optclaw loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="optclaw"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="optclaw" tells you which config dir to use
        ...
```
...
```
packages/harness/optclaw/guardrails/
...
packages/harness/optclaw/config/
    guardrails_config.py     # GuardrailsConfig Pydantic model + singleton

packages/harness/optclaw/agents/middlewares/
