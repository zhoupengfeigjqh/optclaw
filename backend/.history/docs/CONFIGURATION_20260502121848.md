...
This guide explains how to configure optclaw for your environment.
...
- Claude Code OAuth (`optclaw.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`optclaw.models.openai_codex_provider:CodexChatModel`)
...
models:
  - name: gpt-5.4
    ...
    use: optclaw.models.openai_codex_provider:CodexChatModel
    model: gpt-5.4
  ...
  - name: claude-sonnet-4.6
    ...
    use: optclaw.models.claude_provider:ClaudeChatModel
...
- On macOS, optclaw does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `optclaw.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: optclaw.models.patched_openai:PatchedChatOpenAI
...
optclaw supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: optclaw.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: optclaw.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```
...
```yaml
sandbox:
   use: optclaw.community.aio_sandbox:AioSandboxProvider
   provisioner_url: http://provisioner:8002
```

When using Docker development (`make docker-start`), optclaw starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
```yaml
sandbox:
  use: optclaw.sandbox.local:LocalSandboxProvider
  allow_host_bash: false
```

`allow_host_bash` is intentionally `false` by default. optclaw's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: opt-claw-sandbox
...
When you configure `sandbox.mounts`, optclaw exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.
...
**How Skills Work**:
- Skills are stored in `opt-claw/skills/{public,custom}/`
...
2. Restart the optclaw service to apply changes

## Environment Variables

optclaw supports environment variable substitution using the `$` prefix:
...
- `OPT_CLAW_CONFIG_PATH` - Custom config file path

## Configuration Location

The configuration file should be placed in the **project root directory** (`opt-claw/config.yaml`), not in the backend directory.

## Configuration Priority

optclaw searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `OPT_CLAW_CONFIG_PATH` environment variable
3. `config.yaml` in current working directory (typically `backend/` when running)
4. `config.yaml` in parent directory (project root: `opt-claw/`)
...
- Ensure `config.yaml` exists in the **project root** directory (`opt-claw/config.yaml`)
- The backend searches parent directory by default, so root location is preferred
- Alternatively, set `OPT_CLAW_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `opt-claw/skills/` directory exists
