...
Quick setup instructions for optclaw.

## Configuration Setup

optclaw uses a YAML configuration file that should be placed in the **project root directory**.
...
4. **Verify configuration**:
   ```bash
   cd backend
   python -c "from optclaw.config import get_app_config; print('✓ Config loaded:', get_app_config().models[0].name)"
   ```

## Important Notes

- **Location**: `config.yaml` should be in `opt-claw/` (project root), not `opt-claw/backend/`
...
1. `OPT_CLAW_CONFIG_PATH` environment variable (if set)
2. `backend/config.yaml` (current directory when running from backend/)
3. `opt-claw/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`opt-claw/config.yaml`).

## Sandbox Setup (Optional but Recommended)

If you plan to use Docker/Container-based sandbox (configured in `config.yaml` under `sandbox.use: optclaw.community.aio_sandbox:AioSandboxProvider`), it's highly recommended to pre-pull the container image:
...
cd opt-claw/backend
python -c "from optclaw.config.app_config import AppConfig; print(AppConfig.resolve_config_path())"
```
