...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`optclaw/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  optclaw.skills.installer
optclaw.client             ──┘              optclaw.uploads.manager
```

- Shared modules live in the harness layer (`optclaw.*`), pure business logic, no FastAPI dependency
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `OptClawClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `OptClawClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
