...
This document provides a comprehensive overview of the optclaw backend architecture.
...
**Entry Point**: `packages/harness/optclaw/agents/lead_agent/agent.py:make_lead_agent`
...
```json
{
  "agent": {
    "type": "agent",
    "path": "optclaw.agents:make_lead_agent"
  }
}
```
...
- `threads.py` - `/api/threads/{id}` - Local optclaw thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow is now split across both backend surfaces: LangGraph handles `DELETE /api/langgraph/threads/{thread_id}` for thread state, then the Gateway `threads.py` router removes optclaw-managed filesystem data via `Paths.delete_thread_dir()`.
...
│  (packages/harness/optclaw/sandbox/local.py) │              │  (packages/harness/optclaw/community/)       │
│                         │              │                         │
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.opt-claw/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.opt-claw/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.opt-claw/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `opt-claw/skills/` |
...
│  (packages/harness/optclaw/tools/)       │  │  (config.yaml)      │  │  (extensions.json)  │
├─────────────────────┤  ├─────────────────────┤  ├─────────────────────┤
...
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
           ...
           └───────────────────────┴───────────────────────┘
                      ...
                      ┌─────────────────────────┐
                      │   get_available_tools() │
                      │   (packages/harness/optclaw/tools/__init__)  │
                      └─────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Model Factory                                   │
│                     (packages/harness/optclaw/models/factory.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          MCP Integration                                 │
│                        (packages/harness/optclaw/mcp/manager.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Skills System                                   │
│                       (packages/harness/optclaw/skills/loader.py)                             │
└─────────────────────────────────────────────────────────────────────────┘
...
2. Gateway receives file
   - Validates file
   - Stores in .opt-claw/threads/{thread_id}/user-data/uploads/
...
3. Gateway removes local optclaw-managed files
   - Deletes .opt-claw/threads/{thread_id}/ recursively
