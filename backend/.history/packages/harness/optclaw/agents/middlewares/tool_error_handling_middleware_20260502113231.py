...
def _build_runtime_middlewares(
    *,
    ...
    from optclaw.config.guardrails_config import get_guardrails_config
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "optclaw"
    ...
    from optclaw.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
