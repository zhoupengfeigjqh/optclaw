...
Usage (e.g. FastAPI lifespan)::
    ...
    from optclaw.runtime.store import make_store
...
from optclaw.runtime.store.provider import POSTGRES_CONN_REQUIRED, POSTGRES_STORE_INSTALL, SQLITE_STORE_INSTALL, ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_store(config) -> AsyncIterator[BaseStore]:
    ...
    The ``config`` argument is a :class:`optclaw.config.checkpointer_config.CheckpointerConfig`
...
async def make_store() -> AsyncIterator[BaseStore]:
    ...
    :func:`optclaw.agents.checkpointer.async_provider.make_checkpointer` so
