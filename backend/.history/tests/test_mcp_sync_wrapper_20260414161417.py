...
from optclaw.mcp.tools import _make_sync_tool_wrapper, get_mcp_tools
...
def test_mcp_tool_sync_wrapper_generation():
    ...
    with (
        ...
        patch("optclaw.config.extensions_config.ExtensionsConfig.from_file"),
        patch("optclaw.mcp.tools.build_servers_config", return_value={"test-server": {}}),
        patch("optclaw.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
    ):
...
def test_mcp_tool_sync_wrapper_in_running_loop():
    ...
    # Test the real helper function exported from optclaw.mcp.tools
...
def test_mcp_tool_sync_wrapper_exception_logging():
    ...
    with patch("optclaw.mcp.tools.logger.error") as mock_log_error:
