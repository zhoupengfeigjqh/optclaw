...
from optclaw.config.acp_config import ACPAgentConfig
from optclaw.config.extensions_config import ExtensionsConfig, McpServerConfig, set_extensions_config
from optclaw.tools.builtins.invoke_acp_agent_tool import (
...
def test_build_mcp_servers_filters_disabled_and_maps_transports():
    ...
    monkeypatch.setattr(
        "optclaw.config.extensions_config.ExtensionsConfig.from_file",
...
def test_get_work_dir_uses_base_dir_when_no_thread_id(monkeypatch, tmp_path):
    ...
    from optclaw.config import paths as paths_module
...
async def test_invoke_acp_agent_skips_invalid_mcp_servers(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr(paths_module, "get_paths", lambda: paths_module.Paths(base_dir=tmp_path))
    monkeypatch.setattr(
        "optclaw.tools.builtins.invoke_acp_agent_tool._build_acp_mcp_servers",
...
def test_get_available_tools_includes_invoke_acp_agent_when_agents_configured(monkeypatch):
    ...
    monkeypatch.setattr("optclaw.tools.tools.get_app_config", lambda: fake_config)
    monkeypatch.setattr(
        "optclaw.config.extensions_config.ExtensionsConfig.from_file",
        classmethod(lambda cls: ExtensionsConfig(mcp_servers={}, skills={})),
    )
