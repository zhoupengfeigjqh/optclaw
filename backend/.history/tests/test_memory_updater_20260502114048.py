...
from optclaw.agents.memory.prompt import format_conversation_for_update
from optclaw.agents.memory.updater import (
    MemoryUpdater,
    _extract_text,
...
def test_apply_updates_skips_existing_duplicate_and_preserves_removals() -> None:
    ...
    with patch(
        "optclaw.agents.memory.updater.get_memory_config",
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {"content": "User works on optclaw", "category": "context", "confidence": 0.87},
        ],
    }
...
def test_clear_memory_data_resets_all_sections() -> None:
    with patch("optclaw.agents.memory.updater._save_memory_to_file", return_value=True):
...
def test_delete_memory_fact_removes_only_matching_fact() -> None:
    ...
    with (
        patch("optclaw.agents.memory.updater.get_memory_data", return_value=current_memory),
        patch("optclaw.agents.memory.updater._save_memory_to_file", return_value=True),
    ):
...
def test_create_memory_fact_appends_manual_fact() -> None:
    with (
        patch("optclaw.agents.memory.updater.get_memory_data", return_value=_make_memory()),
        patch("optclaw.agents.memory.updater._save_memory_to_file", return_value=True),
    ):
...
def test_delete_memory_fact_raises_for_unknown_id() -> None:
    with patch("optclaw.agents.memory.updater.get_memory_data", return_value=_make_memory()):
        try:
...
def test_import_memory_data_saves_and_returns_imported_memory() -> None:
    ...
    with patch("optclaw.agents.memory.updater.get_memory_storage", return_value=mock_storage):
...
def test_update_memory_fact_rejects_invalid_confidence() -> None:
    ...
    for confidence in (-0.1, 1.1, float("nan"), float("inf"), float("-inf")):
        with patch(
            "optclaw.agents.memory.updater.get_memory_data",
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_string_response_parses(self):
        ...
        with (
            ...
            patch("optclaw.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
            patch("optclaw.agents.memory.updater.get_memory_data", return_value=_make_memory()),
            patch("optclaw.agents.memory.updater.get_memory_storage", return_value=MagicMock(save=MagicMock(return_value=True))),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"
    ...
    def test_sync_update_memory_returns_false_when_bridge_submit_fails(self):
        ...
        with (
            patch(
                "optclaw.agents.memory.updater._SYNC_MEMORY_UPDATER_EXECUTOR.submit",
