...
from optclaw.agents.middlewares.summarization_middleware import OptClawSummarizationMiddleware, SummarizationEvent
...
def _middleware(*, before_summarization=None, trigger=("messages", 4), keep=("messages", 2)) -> OptClawSummarizationMiddleware:
    model = MagicMock()
    model.invoke.return_value = SimpleNamespace(text="compressed summary")
    return OptClawSummarizationMiddleware(
        model=model,
        trigger=trigger,
        keep=keep,
        token_counter=len,
...
def test_memory_flush_hook_skips_when_memory_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    queue = MagicMock()
    monkeypatch.setattr("optclaw.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=False))
    monkeypatch.setattr("optclaw.agents.memory.summarization_hook.get_memory_queue", lambda: queue)
...
def test_memory_flush_hook_skips_when_thread_id_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    queue = MagicMock()
    monkeypatch.setattr("optclaw.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=True))
    monkeypatch.setattr("optclaw.agents.memory.summarization_hook.get_memory_queue", lambda: queue)
