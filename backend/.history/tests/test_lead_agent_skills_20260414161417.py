...
from optclaw.agents.lead_agent.prompt import get_skills_prompt_section
...
def test_get_skills_prompt_section_returns_empty_when_no_skills_match(monkeypatch):
    ...
    monkeypatch.setattr("optclaw.agents.lead_agent.prompt._get_enabled_skills", lambda: skills)
...
def test_get_skills_prompt_section_includes_self_evolution_rules_without_skills(monkeypatch):
    monkeypatch.setattr("optclaw.agents.lead_agent.prompt._get_enabled_skills", lambda: [])
    monkeypatch.setattr(
        "optclaw.config.get_app_config",
        lambda: SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills"),
...
def test_get_skills_prompt_section_cache_respects_skill_evolution_toggle(monkeypatch):
    ...
    monkeypatch.setattr("optclaw.config.get_app_config", lambda: config)
...
def test_make_lead_agent_empty_skills_passed_correctly(monkeypatch):
    ...
    from optclaw.agents.lead_agent import agent as lead_agent_module
    ...
    monkeypatch.setattr("optclaw.tools.get_available_tools", lambda **kwargs: [])
