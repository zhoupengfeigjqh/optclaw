"""End-to-end tests for OptClawClient.
...
Only OPT_CLAW_HOME is redirected to tmp_path for filesystem isolation.
...
from optclaw.config.app_config import AppConfig
...
def _make_e2e_config() -> AppConfig:
    ...
    return AppConfig(
        ...
        sandbox=SandboxConfig(use="optclaw.sandbox.local:LocalSandboxProvider", allow_host_bash=True),
    )
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - OPT_CLAW_HOME → tmp_path (all thread data lands in a temp dir)
    ...
    monkeypatch.setenv("OPT_CLAW_HOME", str(tmp_path))
    monkeypatch.setattr("optclaw.config.paths._paths", None)
    monkeypatch.setattr("optclaw.sandbox.sandbox_provider._default_sandbox_provider", None)

    # 2. Inject a clean AppConfig via the global singleton.
    config = _make_e2e_config()
    monkeypatch.setattr("optclaw.config.app_config._app_config", config)
    monkeypatch.setattr("optclaw.config.app_config._app_config_is_custom", True)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from optclaw.config.title_config import TitleConfig

    monkeypatch.setattr("optclaw.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from optclaw.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "optclaw.agents.middlewares.memory_middleware.get_memory_config",
    ...
    monkeypatch.setattr("optclaw.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from optclaw.agents.lead_agent.agent import _build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("optclaw.client._build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A OptClawClient wired to the isolated e2e_env."""
    return OptClawClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = OptClawClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = OptClawClient(checkpointer=None, plan_mode=False)
        ...
        c2 = OptClawClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "optclaw.skills.installer.get_skills_root_path",
            lambda: skills_root,
        )
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPT_CLAW_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from optclaw.config.extensions_config import reload_extensions_config
