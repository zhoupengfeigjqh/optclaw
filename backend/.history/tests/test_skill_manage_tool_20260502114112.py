...
skill_manage_module = importlib.import_module("optclaw.tools.skill_manage_tool")
...
async def _async_result(decision: str, reason: str):
    from optclaw.skills.security_scanner import ScanResult
...
def test_skill_manage_create_and_patch(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("optclaw.skills.manager.get_app_config", lambda: config)
    monkeypatch.setattr("optclaw.skills.security_scanner.get_app_config", lambda: config)
    refresh_calls = []
