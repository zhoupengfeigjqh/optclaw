...
from optclaw.config.paths import Paths, join_host_path
...
def test_join_host_path_preserves_windows_drive_letter_style():
    base = r"C:\Users\demo\opt-claw\backend\.opt-claw"

    joined = join_host_path(base, "threads", "thread-9", "user-data", "outputs")

    assert joined == r"C:\Users\demo\opt-claw\backend\.opt-claw\threads\thread-9\user-data\outputs"
...
def test_get_thread_mounts_preserves_windows_host_path_style(tmp_path, monkeypatch):
    ...
    aio_mod = importlib.import_module("optclaw.community.aio_sandbox.aio_sandbox_provider")
    monkeypatch.setenv("OPT_CLAW_HOST_BASE_DIR", r"C:\Users\demo\opt-claw\backend\.opt-claw")
    monkeypatch.setattr(aio_mod, "get_paths", lambda: Paths(base_dir=tmp_path))
    ...
    assert container_paths["/mnt/user-data/workspace"] == r"C:\Users\demo\opt-claw\backend\.opt-claw\threads\thread-10\user-data\workspace"
    assert container_paths["/mnt/user-data/uploads"] == r"C:\Users\demo\opt-claw\backend\.opt-claw\threads\thread-10\user-data\uploads"
    assert container_paths["/mnt/user-data/outputs"] == r"C:\Users\demo\opt-claw\backend\.opt-claw\threads\thread-10\user-data\outputs"
    assert container_paths["/mnt/acp-workspace"] == r"C:\Users\demo\opt-claw\backend\.opt-claw\threads\thread-10\acp-workspace"
