...
optclaw.subagents.executor. This test file uses delayed import via fixture to test
...
_MOCKED_MODULE_NAMES = [
    "optclaw.agents",
    "optclaw.agents.thread_state",
    "optclaw.agents.middlewares",
    "optclaw.agents.middlewares.thread_data_middleware",
    "optclaw.sandbox",
    "optclaw.sandbox.middleware",
    "optclaw.sandbox.security",
    "optclaw.models",
]
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("optclaw.subagents.executor")

    # Remove mocked executor if exists (from conftest.py)
    if "optclaw.subagents.executor" in sys.modules:
        del sys.modules["optclaw.subagents.executor"]
    ...
    from optclaw.subagents.executor import (
        SubagentExecutor,
        SubagentResult,
        SubagentStatus,
    )
    ...
    if original_executor is not None:
        sys.modules["optclaw.subagents.executor"] = original_executor
    elif "optclaw.subagents.executor" in sys.modules:
        del sys.modules["optclaw.subagents.executor"]
...
class TestCleanupBackgroundTask:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        from optclaw.subagents import executor
...
class TestCooperativeCancellation:
    ...
    def test_timeout_does_not_overwrite_cancelled(self, executor_module, classes, base_config, msg):
        ...
        def blocking_execute(task, result_holder=None):
            ...
            from optclaw.subagents.executor import SubagentResult as _R
