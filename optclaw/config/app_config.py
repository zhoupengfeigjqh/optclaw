...
class AppConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `OPT_CLAW_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("OPT_CLAW_CONFIG_PATH"):
            path = Path(os.getenv("OPT_CLAW_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `OPT_CLAW_CONFIG_PATH` not found at {path}")
            return path
        else:
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("optclaw_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("optclaw_current_app_config_stack", default=())
...
def get_app_config() -> AppConfig:
    """Get the optclaw config instance.
