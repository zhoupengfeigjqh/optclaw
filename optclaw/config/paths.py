...
def _default_local_base_dir() -> Path:
    """Return the repo-local optclaw state directory without relying on cwd."""
    backend_dir = Path(__file__).resolve().parents[1]
    return backend_dir / ".opt-claw"
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.opt-claw``).  Using
...
class Paths:
    """
    Centralized path configuration for optclaw application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. OPT_CLAW_HOME environment variable
        3. Repo-local fallback derived from this module path: `{backend_dir}/.opt-claw`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set OPT_CLAW_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("OPT_CLAW_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("OPT_CLAW_HOME"):
