...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Note: if you previously set this to `.opt-claw/memory.json`, "
            "the file will now be resolved as `{base_dir}/.opt-claw/memory.json`; "
            "migrate existing data or use an absolute path to preserve the old location."
        ),
    )
    storage_class: str = Field(
        default="optclaw.agents.memory.storage.FileMemoryStorage",
