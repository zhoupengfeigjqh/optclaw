...
@tool("execute_python_file", parse_docstring=True)
def execute_python_file_tool(path: str, timeout: int = _DEFAULT_TIMEOUT) -> str:
    """Execute a specific Python file and return its standard output and error.
    ...
    try:
        ...
        # 使用当前运行环境的 python 解释器（如果是 venv，会保持在 venv 内）
        import sys
        python_executable = sys.executable

        result = subprocess.run(
            [python_executable, actual_path],
        ...
        return f"Error: Execution timed out after {timeout} seconds. The process was terminated."
    except OSError as e:
